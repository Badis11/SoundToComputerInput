...
class APU:
    ...
    mic_index = 52 #WAŻNE dodać ustawienia do mikrofonu
    ...
    def calcFrequency(self):
        ...
        if (self.stream):
            ...
            denom, numer = scipy.signal.butter(self.BUTTERWORTH_ORDER,
                                               [self.high_pass / nyquist, self.low_pass / nyquist], btype="band")
    ...
    def start(self):
        ...
        self.stream = self.getPyAudio().open(format=pyaudio.paFloat32, channels=self.MONO,
                                             input_device_index=self.mic_index, rate=self.sampling_rate, input=True,
